    ...
    public ServerResponse setChangeWorker(String userToken, String houseWorkerId) {
        try {
            ...
            if(!CommonUtil.isEmpty(workerId)){
                ...
                configMessageService.addConfigMessage(null,"gj",workerId,"0","业主换人提醒",String.format(DjConstants.PushMessage.STEWARD_REPLACE,house.getHouseName()) ,"");
            }
...
//            3大管家,4拆除，5打孔，6水电工，7防水，8泥工,9木工，10油漆工，11安装
            //通知业主设计师抢单成功
            if (worker.getWorkerType() == 1) {//设计师
                configMessageService.addConfigMessage(null,"zx",house.getMemberId(),"0","设计师抢单提醒",String.format(DjConstants.PushMessage.DESIGNER_GRABS_THE_BILL,house.getHouseName()) ,"");
            }
            //通知业主大管家抢单成功
            if (worker.getWorkerType() == 3) {//大管家
                configMessageService.addConfigMessage(null,"zx",house.getMemberId(),"0","大管家抢单提醒",String.format(DjConstants.PushMessage.STEWARD_RUSH_TO_PURCHASE,house.getHouseName()) ,"");
            }
            if (worker.getWorkerType() >3 ) {//其他工匠
                configMessageService.addConfigMessage(null,"zx",house.getMemberId(),"0","工匠抢单提醒",String.format(DjConstants.PushMessage.CRAFTSMAN_RUSH_TO_PURCHASE,house.getHouseName()) ,"");
    ...
    public ServerResponse getConstructionByWorkerId(String userToken, String cityId) {
        try {
            ...
            if (worker.getWorkerType() == 3) {//大管家
                ...
                List<ConstructionByWorkerIdBean.WokerFlowListBean> wokerFlowList = new ArrayList<>();
                ...
                if (hf.getWorkerType() == 3 && hf.getWorkType() == 4 && hf.getSupervisorStart() == 1) {//当业主支付大管家费用并且确认开工之后之后才出现
                    for (HouseFlow hfl : hfList) {
                        ...
                        wokerFlowList.add(wfr);
                    }
                }
                bean.setHouseIsStart(houseIsStart ? "今日已开工" : "今日未开工");
                bean.setWokerFlowList(wokerFlowList);
                int count = 0;
                ...
                Member wokerSup = memberMapper.selectByPrimaryKey(supervisorWorker == null ? "" : supervisorWorker.getWorkerId());//查询大管家
                bean.setSupervisorName(wokerSup == null ? "无" : wokerSup.getName());//大管家名字
                bean.setSupervisorPhone(wokerSup == null ? "无" : wokerSup.getMobile());
                bean.setSupervisorEvation("积分 " + (wokerSup == null ? "0.00" : wokerSup.getEvaluationScore()));//大管家积分
                Long supervisorCountOrder = houseWorkerMapper.getCountOrderByWorkerId(wokerSup == null ? "" : wokerSup.getId());
                bean.setSupervisorCountOrder("总单数 " + (supervisorCountOrder == null ? "0" : supervisorCountOrder));//大管家总单数
                bean.setSupervisorPraiseRate("好评率 " + ((wokerSup != null ? wokerSup.getPraiseRate().multiply(new BigDecimal(100)) : 0) + "%"));//大管家好评率
                if (hf.getWorkType() == 3) {//如果是已抢单待支付。则提醒业主支付
     ...
     * @return 菜单列表
     */
    private List<ConstructionByWorkerIdBean.BigListBean> getBigList(String userToken, String cityId, House house, Member worker) {
     ...
     * @param promptList   消息
     * @param buttonList   按钮
     * @param checkFlowApp 根据工种任务id和工人id查询此工人待审核
    ...
    private void setDisplayState(HouseFlow hf, List<String> promptList, List<ConstructionByWorkerIdBean.ButtonListBean> buttonList, HouseFlowApply checkFlowApp, boolean isShow) {
        if (isShow) {
            if (checkFlowApp != null && checkFlowApp.getApplyType() == 2 && checkFlowApp.getSupervisorCheck() == 0) {
                ...
                promptList.add("大管家审核不通过,请重新提交整体完工申请");
            ...
            if (checkFlowApp != null && checkFlowApp.getApplyType() == 1 && checkFlowApp.getSupervisorCheck() == 0) {
                ...
                promptList.add("大管家审核不通过,请重新提交阶段完工申请");
                buttonList.add(getButton("申请阶段完工", 4));
            } else if (hf.getWorkerType() != 1) {
                buttonList.add(getButton("申请阶段完工", 4));
            }
        }
    }
        ...
        try {
            ...
            if (supervisorHF.getPatrolMoney() == null || supervisorHF.getCheckMoney() == null ||
                ...
                for (HouseFlow houseflow : houseFlowList) {
                    ...
                    ServerResponse serverResponse = workerGoodsAPI.getWorkertoCheck(request, houseFlow.getHouseId(), houseflow.getId());
                    if (serverResponse.isSuccess()) {
                ...
                if (supervisorHF.getWorkPrice().compareTo(new BigDecimal(0)) == 0) {
                    request.setAttribute(Constants.CITY_ID, houseFlow.getCityId());
                    ServerResponse serverResponse = workerGoodsAPI.getWorkertoCheck(request, houseFlow.getHouseId(), supervisorHF.getId());
                    if (serverResponse.isSuccess()) {
                        JSONObject obj = JSONObject.parseObject(serverResponse.getResultObj().toString());
            ...
            if (applyType != 5 && houseFlowApplyMapper.waitHouseFlowApply(houseFlowId, workerId).size() > 0) {
                //监理已审核通过,等待业主审核
            ...
            if (applyType == 0) {//每日完工申请
                ...
                configMessageService.addConfigMessage(null,"gj",supervisorHF.getWorkerId(),"0","阶段完工申请",String.format(DjConstants.PushMessage.STEWARD_APPLY_FINISHED,house.getHouseName(),workType.getName()) ,"");
                ...
                configMessageService.addConfigMessage(null,"gj",supervisorHF.getWorkerId(),"0","整体完工申请",String.format(DjConstants.PushMessage.STEWARD_APPLY_FINISHED,house.getHouseName(),workType.getName()) ,"");
                ...
                configMessageService.addConfigMessage(null,"gj",supervisorHF.getWorkerId(),"0","工匠申请停工",String.format(DjConstants.PushMessage.STEWARD_CRAFTSMEN_APPLY_FOR_STOPPAGE,house.getHouseName()) ,"");
                return ServerResponse.createBySuccessMessage("操作成功");
                //***每日开工申请(不用审核，默认审核通过)***//
            } else if (applyType == 4) {
            ...
            for (int i = 0; i < imageArr.size(); i++) {//上传材料照片
                ...
                for (String anImageUrlList : imageUrlList) {
                    HouseFlowApplyImage hfai = new HouseFlowApplyImage();
                    hfai.setImageUrl(anImageUrlList);
                    hfai.setImageType(imageObj.getInteger("imageType"));//图片类型 0：材料照片；1：进度照片；2:现场照片；3:其他
                ...
                /****************修改验收节点状态为已验收***************/
                if ((imageObj.getInteger("imageType") != null && imageObj.getInteger("imageType") == 3) &&
                        (imageObj.getString("imageTypeId") != null && !"".equals(imageObj.getString("imageTypeId")))) {
                    TechnologyRecord technologyRecord = new TechnologyRecord();
                    technologyRecord.setState(1);//验收状态0:未验收;1:已验收
                    Example example1 = new Example(TechnologyRecord.class);
                    example1.createCriteria().andEqualTo(TechnologyRecord.TECHNOLOGY_ID, imageObj.getString("imageTypeId"));
                    technologyRecordMapper.updateByExampleSelective(technologyRecord, example1);
                }
            }
