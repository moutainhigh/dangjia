...
//            }
    ...
    public ServerResponse checkForgotPasswordCode(String phone, int smscode) {
        ...
        user = memberMapper.getUser(user);
        if (user == null) {
            return ServerResponse.createByErrorCodeMessage(ServerCode.ERROR.getCode(), "验证码错误！");
        } else {
    ...
    public ServerResponse updateForgotPassword(String phone, String password, String token) {
        if (CommonUtil.isEmpty(token)) {
            return ServerResponse.createByErrorCodeMessage(ServerCode.ERROR.getCode(), "身份认证错误,无认证参数！");
        }
        Member user = new Member();
        user.setMobile(phone);
        user = memberMapper.getUser(user);
        if (user == null) {
            return ServerResponse.createByErrorCodeMessage(ServerCode.ERROR.getCode(), "电话号码未注册！");
        } else {
            if (user.getCheckType() ==4) {
                //冻结的帐户不能修改资料信息
                return ServerResponse.createByErrorMessage("账户冻结，无法修改资料");
            ...
            if (!token.equals(mytoken)) {
                return ServerResponse.createByErrorCodeMessage(ServerCode.ERROR.getCode(), "身份认证错误，身份不匹配！");
            }
            //认证通过，清除token认证
    ...
    public ServerResponse getAccessTokenByUserToken(String userToken) {
        ...
        if (accessToken == null) {//无效的token
            ...
            if (flag) {//失效
                return ServerResponse.createByErrorCodeMessage(ServerCode.USER_TOKEN_ERROR.getCode(), "token已失效,请重新登录！");
            } else {
