...
public interface WorkerGoodsAPI {
    ...
    public ServerResponse<PageInfo> getWorkerGoodses(@RequestParam("pageDTO") PageDTO pageDTO,@RequestParam("workerTypeId")String workerTypeId, @RequestParam("searchKey")String searchKey,@RequestParam("showGoods")String showGoods) ;

    @PostMapping("/basics/workerGoods/setWorkerGoods")
    @ApiOperation(value = "新增或更新工价商品", notes = "新增或更新工价商品")
    public ServerResponse setWorkerGoods(@RequestParam("workerGoods")WorkerGoods workerGoods, @RequestParam("technologyIds")String technologyIds);

    @PostMapping("/basics/workerGoods/getWorkertoCheck")
    @ApiOperation(value = "每工种未删除或已支付工钱", notes = "每工种未删除或已支付工钱")
    public ServerResponse getWorkertoCheck(@RequestParam("houseId")String houseId,@RequestParam("houseFlowId")String houseFlowId);

    @PostMapping("/basics/workerGoods/getPayedWorker")
    @ApiOperation(value = " 从精算表查工种已支付工钱", notes = " 从精算表查工种已支付工钱")
    public ServerResponse getPayedWorker(@RequestParam("houseId")String houseId,@RequestParam("houseFlowId")String houseFlowId);
