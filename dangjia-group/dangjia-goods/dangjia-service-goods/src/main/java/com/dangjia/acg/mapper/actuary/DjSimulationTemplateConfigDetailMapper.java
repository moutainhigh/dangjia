...
public interface DjSimulationTemplateConfigDetailMapper extends Mapper<DjSimulationTemplateConfigDetail> {
     ...
     * 修改临时数据的状态为正式数据
     */
    void updateTemplateDetailConfig(@Param("cityId") String cityId,@Param("serviceTypeId") String serviceTypeId);

    /**
     * 添加一批新的临时数据(和正式数据保持一致）
     */
    void batchInsertTempateDetail(@Param("cityId") String cityId,@Param("serviceTypeId") String serviceTypeId);
