...
/**
 * @Description:根据浏览案例-进行推荐
 * @author: luof
 * @date: 2020-3-11
 */
@Service
public class BrowseCaseRecommend {

    /** 声明日志 */
    private static Logger logger = LoggerFactory.getLogger(BrowseCaseRecommend.class);

    /** 加载推荐 */
    public ServerResponse loadRecommendTarget(String memberId, PageDTO pageDTO){

        int number = pageDTO.getPageSize();
        logger.debug("需加载[浏览案例]推荐条数["+number+"]");
        if( number < 1 ){
            return ServerResponse.createByErrorCodeMessage(ServerCode.NO_DATA.getCode(), "查无数据");
        }

        // 浏览案例推荐 TODO 返回空 暂不处理
