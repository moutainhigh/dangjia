...
public class AccessFilter extends ZuulFilter {
  ...
  public Object run() {
    try {
      ...
      if (transCode.equals(Constants.USER_LOGIN)) {
        return null;
      }else if(transCode.equals(Constants.USER_LOGOUT)){
        return null;
      }else if(transCode.equals(Constants.USER_ADDUSERLOG)){
        return null;
      }
      context.getResponse().setHeader("Access-Control-Allow-Origin","*");
      context.getResponse().setHeader("Access-Control-Allow-Origin","*");
      // 响应类型
      context.getResponse().setHeader("Access-Control-Allow-Methods", "POST, GET, DELETE, OPTIONS, DELETE");
      // 响应头设置
      context.getResponse().setHeader("Access-Control-Allow-Headers", "Content-Type, x-requested-with, X-Custom-Header, HaiYi-Access-Token");
      if ("OPTIONS".equals(request.getMethod())){
        context.getResponse().setStatus(HttpStatus.SC_NO_CONTENT);
      }
      //获取入参
...
//      }
      ...
      String responseInfo = StreamUtils.copyToString(stream, Charset.forName("UTF-8"));
      // 不加前端接收不到返回
